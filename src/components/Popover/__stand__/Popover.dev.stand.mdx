...
Popover позиционируется по абсолютным отступам `top`, `left`, `bottom`, `right`.
Указать желаемые координаты можно через свойства `positionOnX` и `positionOnY`.

Визуально отличий нет, но это может понадобиться для применения отступов средствами css.
Например, `margin-right` не сработает, если позиционирование сделано по `left`.
...
| [`positionOnX?`](#желаемые-координаты)               | `'left' &#124; 'right'`                                              | 'left'                  | Использовать координаты по x по левому или правому краю контента                                            |
| [`positionOnXY`](#желаемые-координаты)               | `'top' &#124; 'bottom'`                                              | 'top'                   | Использовать координаты по y по верхнему или нижнему краю контента                                          |
